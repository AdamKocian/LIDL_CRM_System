          ...
          <table class="table m-0 table-hover">
            ...
            <tbody>
              ...
              while ($row = $qry->fetch_assoc()) :
                $prog = 0;
                $tprog = $conn->query("SELECT * FROM category_list where category_list_id = {$row['id']}")->num_rows;
                $cprog = $conn->query("SELECT * FROM category_list where category_list_id = {$row['id']}")->num_rows; // and status = 3
  ...
  <div class="col-md-4">
    <div class="row">
      ...
      <div class="col-12 col-sm-6 col-md-12">
        <div class="small-box bg-light shadow-sm border">
          <div class="inner">
            <h3><?php echo $conn->query("SELECT t.*,p.name as pname,p.start_date,p.status as pstatus, p.end_date,p.id as pid FROM category_list t inner join team_list p on p.id = t.category_list_id $where2")->num_rows; ?></h3>
            <p>Počet úloh</p>
          </div>
          <div class="icon">
