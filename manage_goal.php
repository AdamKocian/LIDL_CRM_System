...
if (isset($_GET['id'])) {
	$qry = $conn->query("SELECT * FROM goals_list where id = " . $_GET['id'])->fetch_array();
	foreach ($qry as $k => $v) {
		$$k = $v;
	}
}
	...
	<form action="" id="manage-task">
		...
		<div class="form-group">
			...
			<select name="status" id="status" class="custom-select custom-select-sm">
				<option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>Čaká sa</option>
				<option value="2" <?php echo isset($status) && $status == 2 ? 'selected' : '' ?>>Trvá</option>
