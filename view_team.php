...
$tprog = $conn->query("SELECT * FROM category_color where project_id = {$id}")->num_rows;
$cprog = $conn->query("SELECT * FROM category_color where project_id = {$id}")->num_rows; // and status = 3
					...
					<div class="table-responsive">
						<table class="table table-condensed m-0 table-hover">
							...
							<tbody>
								<?php
								$i = 1;
								$tasks = $conn->query("SELECT * FROM category_color where project_id = {$id} order by task asc");
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card">
				...
				<div class="card-body">
					<?php
					$progress = $conn->query("SELECT p.*,concat(u.firstname,' ',u.lastname) as uname,u.avatar,t.task FROM task_list p inner join users u on u.id = p.user_id inner join category_color t on t.id = p.task_id where p.project_id = $id order by unix_timestamp(p.date_created) desc ");
