		...
		<div class="card-body">
			<table class="table tabe-hover table-condensed" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					$stat = array("ČAKÁ SA", "ZAČAL", "PREBIEHA", "POZASTAVENÉ", "PO SPLATNOSTI", "SPLNENÉ");
					$where = "";
					...
					while ($row = $qry->fetch_assoc()) :
						...
						$cprog = $conn->query("SELECT * FROM category_list where category_list_id = {$row['id']} and status = 3")->num_rows;
...
<script>
	$(document).ready(function() {
		...
		$('.delete_project').click(function() {
			_conf("Naozaj chcete vymazať tento tím?", "delete_project", [$(this).attr('data-id')])
		})
	})
	...
	function delete_project($id) {
		...
		$.ajax({
			...
			success: function(resp) {
				if (resp == 1) {
					alert_toast("Údaje boli úspešne vymazané", 'success')
					setTimeout(function() {
						location.reload()
					}, 1500)
